...
// public:
//   void add(int32_t inner_inner_id, int id, uint64_t addr, addr_info loc){
//     std::unique_lock lk(*mtx[id], std::defer_lock);
//     lk.lock();
//     map[id][addr] = loc;
//     lk.unlock();
//   }
//   void erase(int32_t inner_inner_id, int id, uint64_t addr){
//     std::unique_lock lk(*mtx[id], std::defer_lock);
//     lk.lock();
//     map[id].erase(addr);
//     lk.unlock();
//   }
//   std::pair<bool, addr_info> query(int32_t inner_inner_id, int id, uint64_t addr){
//     addr_info info;
//     bool count;
//     std::unique_lock lk(*mtx[id], std::defer_lock);
//     lk.lock();
...
public:
  ...
  void add_size(uint32_t size, bool uncached){
