...
void SimpleTracer::read(uint64_t cache_id, uint64_t addr, int32_t ai, int32_t s, int32_t w, bool hit) {
  std::cout << (read_fmt % UniqueID::name(cache_id) % addr % ai % s % w % hit) << std::endl;
}

void SimpleTracer::write(uint64_t cache_id, uint64_t addr, int32_t ai, int32_t s, int32_t w, bool hit) {
  std::cout << (write_fmt % UniqueID::name(cache_id) % addr % ai % s % w % hit) << std::endl;
}

void SimpleTracer::invalid(uint64_t cache_id, uint64_t addr, int32_t ai, int32_t s, int32_t w) {
