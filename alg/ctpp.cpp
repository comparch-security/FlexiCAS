...
int main(int argc, char **argv) {
  ...
  for(int testN=0; testN<samples; testN++) {
    ...
    do {
      ...
      probe_set.clear();
      for(auto addr:prime_set) {
        if(check_vbuf_hit ? !llc->hit(addr) : !llc->hit(addr, &dai, &ds, &dw, 1, false)) probe_set.push_back(addr);
        if(prefetch_rand_addr)  mem_prefetch(addr);
        else                    mem_access(addr);
      }
      ...
      // do the prune
      evset.clear();
      for(auto addr:probe_set) {
        if(check_vbuf_hit ? llc->hit(addr) : llc->hit(addr, &dai, &ds, &dw, 1, false)) evset.push_back(addr);
        if(prefetch_rand_addr)  mem_prefetch(addr);
        else                    mem_access(addr);
      }
      ...
      int evset_size = 0;
      for(auto addr:evset) {
        if(llc->query_coloc(target, addr)) evset_size++;
      }
    ...
    std::cout << ", collected " << evset_size << " cloc addrs in the final " << evset.size() << " addresses evset.";
    std::cout << " [" << wall_clock - wall_clock_start
